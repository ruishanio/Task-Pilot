 ...
 * Admin OpenAPI 默认实现。
 *
 * 继续把 callback/registry 交给线程组件处理，自动注册能力则收敛到专用服务，避免协议适配层侵入业务细节。
 */
@Service
class AdminBizImpl : AdminBiz {
    ...
    override fun autoRegister(autoRegisterRequest: AutoRegisterRequest): Response<String> =
        taskPilotAutoRegisterService.autoRegister(autoRegisterRequest)
}
