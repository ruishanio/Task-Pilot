    ...
    private fun buildSyncRequest(): SyncRequest {
        ...
        for (definition in TaskPilotMethodScanner.scan(applicationContext, properties.executor.excludedpackage)) {
            ...
            for (registerMetadata in registerMetadataSet) {
                ...
                task.scheduleType = registerMetadata.type
                task.scheduleConf = registerMetadata.conf
                task.misfireStrategy = registerMetadata.misfire
                task.executorRouteStrategy = registerMetadata.route
                task.executorParam = registerMetadata.param
                task.executorBlockStrategy = registerMetadata.block
                task.executorTimeout = registerMetadata.timeout
